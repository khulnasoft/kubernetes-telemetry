...
be used in the  [Khulnasoft Kubernetes Ingress Controller (KIC)][kic] or other
...
[kic]:https://github.com/khulnasoft/kubernetes-ingress-controller
[go]:https://go.dev
...
- We want to detect if our users deploy service mesh in their kubernetes
  ...
  linkerd, or kuma)
- We want to detect if [KIC][kic] and/or [Khulnasoft Gateway][khulnasoftgw] is running
...
[kic]:https://github.com/khulnasoft/kubernetes-ingress-controller
[khulnasoftgw]:https://docs.khulnasofthq.com/gateway/

### Goals
- Implement detection method as a golang library for the follwing 7 kinds of
  service meshes: `istio`,`linkerd`,`kuma`,`khulnasoft mesh`,`consul connect`,
...
|Mesh Name| istio  | linkerd   	| kuma   	|   khulnasoft mesh	| consul connect | traefik mesh | aws appmesh |
|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:------:|
|signal1: namespace	|`istio-system` 	|`linkerd` |`kuma-system`  	| `khulnasoft-mesh-system`  	|`consul`| `traefik-mesh`|`appmesh-system`|
...
|   signal3: service(one example) 	|   `istiod`	| `linkerd-proxy-injector`  	|   `kuma-control-plane`	| `khulnasoft-mesh-control-plane` | `consul-server` | `traefik-mesh-controller` | `appmesh-controller-webhook-service` |

Special notes:

- All the service meshes could be installed in a namespace other than the
  namespace in the table.
- Namespace for kuma/khulnasoft mesh has label `kuma.io/system-namespace: "true"`
- kuma and khulnasoft mesh could be distinguished by namespace name and service 
  name.
...
- kuma/khulnasoft mesh can label a `deployment` instead of `namespace` to inject a
  pod.
- kuma/khulnasoft mesh does not inject an init container if kuma CNI is deployed.
...
```
uptime=120;v=2.3.1;k8sv=1.23.1;db=none;id=1234-5678-abcd-dcba;hn=khulnasoft-test
```
...
|Mesh Name| istio  | linkerd   	| kuma   	|   khulnasoft mesh	| consul connect | traefik mesh | aws appmesh |
|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:------:|
| singnal1: namespace | `istio1` or `i1` | `linkerd1` or `l1` | `kuma1` or `k1` | `khulnasoftmesh1` or `km1` | `consul1` or `c1` | `traefik1` or `t1` | `aws1` or `a1` | 
| singnal2: CRD | `istio2` or `i2` | `linkerd2` or `l2` | `kuma2` or `k2` | `khulnasoftmesh2` or `km2` | `consul2` or `c2` | None(No unique CRD) | `aws2` or `a2` |
| singnal3: service | `istio3` or `i3` | `linkerd3` or `l3` | `kuma3` or `k3` | `khulnasoftmesh3` or `km3` | `consul3` or `c3` | `traefik3` or `t3` | `aws3` or `a3` |
...
|Mesh Name| istio  | linkerd   	| kuma   	|   khulnasoft mesh	| consul connect | traefik mesh | aws appmesh |
|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:------:|
| singnal1: namespace label/annotation | `istio1` or `i1` | `linkerd1` or `l1` | `kuma1` or `k1` | `khulnasoftmesh1` or `km1` | `consul1` or `c1` | (NO such annotation) | `aws1` or `a1` | 
| singnal2: pod/service annotation | `istio2` or `i2` | `linkerd2` or `l2` | `kuma2` or `k2` | `khulnasoftmesh2` or `km2` | `consul2` or `c2` | `traefik2` or `t2` | `aws2` or `a2` |
| singnal3: sidecar container in pod | `istio3` or `i3` | `linkerd3` or `l3` | `kuma3` or `k3` | `khulnasoftmesh3` or `km3` | `consul3` or `c3` | (NO sidecar) | `aws3` or `a3` |
| singnal4: init container in pod | `istio4` or `i4` | `linkerd4` or `l4` | `kuma4` or `k4` | `khulnasoftmesh4` or `km4` | `consul4` or `c4` | (NO sidecar) | `aws4` or `a4` |
